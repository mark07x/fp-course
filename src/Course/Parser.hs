...
character ::
  Parser Char
character = P f where f (c :. xc) = Result xc c; f _ = UnexpectedEof
...
instance Functor Parser where
  (<$>) ::
    ...
    -> Parser a
    -> Parser b
  g <$> P f = P $ (flip onResult) (\s a -> Result s $ g a) <$> f
...
(|||) ::
  ...
  -> Parser a
P a ||| P b = P f
              where f s | isErrorResult $ a s = b s
                        | otherwise = a s
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser a
    -> Parser b
  g =<< P f = P (\s -> onResult (f s) (\s' a -> parse (g a) s'))
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy b = P f where f Nil = UnexpectedEof
                      f (x :. xs) | b x = Result xs x
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser = foldRight (\a b -> (:.) <$> a <*> b) (pure Nil)
...
thisMany ::
  ...
  -> Parser a
  -> Parser (List a)
thisMany x = sequenceParser . replicate x
...
phoneParser ::
  Parser Chars
phoneParser = lift2 (:.) digit $ lift2 (++) phoneBodyParser ("" <$ is '#')
...
personParser ::
  Parser Person
personParser = f <$> ageParser <*> spaces1
               <*> firstNameParser <*> spaces1
               <*> surnameParser <*> spaces1
               <*> smokerParser <*> spaces1
               <*> phoneParser
                 where f a1 _ a2 _ a3 _ a4 _ a5 = Person a1 a2 a3 a4 a5
