...
findLeft f (ListZipper (x :. xs) a r) | f x       = IsZ $ ListZipper xs x (a :. r)
                                      | otherwise = findLeft f $ ListZipper xs x (a :. r)
...
findRight f (ListZipper l a (x :. xs)) | f x       = IsZ $ ListZipper (a :. l) x xs
                                       | otherwise = findRight f $ ListZipper (a :. l) x xs
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop z@(ListZipper Nil _ _) = farLoop z
  where farLoop z'@(ListZipper _ _ Nil) = z'
        farLoop (ListZipper l a (x :. xs)) = farLoop $ ListZipper (a :. l) x xs
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop z@(ListZipper _ _ Nil) = farLoop z
  where farLoop z'@(ListZipper Nil _ _) = z'
        farLoop (ListZipper (x :. xs) a r) = farLoop $ ListZipper xs x (a :. r)
...
moveLeftN n z | n == 0 = IsZ z
              | n > 0 = fromOptional $ (toOptional . moveLeftN (n - 1)) P.=<< (toOptional $ moveLeft z)
...
moveRightN n z | n == 0 = IsZ z
               | n > 0 = fromOptional $ (toOptional . moveRightN (n - 1)) P.=<< (toOptional $ moveRight z)
...
-- /Tip:/ Use `zipWith`
  ListZipper l a r <*> ListZipper l' a' r' = ListZipper (zipWith ($) l l')
                                                        (a a')
                                                        (zipWith ($) r r')
...
instance Extend ListZipper where
  f <<= z = f <$> ListZipper (unfoldr kl z) z (unfoldr kr z)
            where kl z' = lift2 (,) (toOptional $ moveLeft z') (toOptional $ moveLeft z')
                  kr z' = lift2 (,) (toOptional $ moveRight z') (toOptional $ moveRight z')
