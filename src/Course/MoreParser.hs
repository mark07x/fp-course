...
string (s :. xs) = lift2 (:.) (is s) (string xs)
...
between ::
  ...
  -> Parser a
  -> Parser a
between o c a = lift3 f o a c where f _ a' _ = a'
...
hex ::
  Parser Char
hex = thisMany 4 character >>= readAscii
        where readAscii s = case readHexs s of
                              Full (x, Nil) -> pure $ chr x
...
sepby1 ::
  ...
  -> Parser s
  -> Parser (List a)
sepby1 e s = lift2 (:.) e $ list ((flip const) <$> s <*> e)
...
sepby ::
  ...
  -> Parser s
  -> Parser (List a)
sepby e s = sepby1 e s ||| valueParser Nil
...
eof ::
  Parser ()
eof = P (\s -> if s == Nil then Result Nil () else ExpectedEof s)
...
satisfyAll ::
  List (Char -> Bool)
  -> Parser Char
satisfyAll = satisfy . (foldRight (lift2 (&&)) (pure True))
...
satisfyAny ::
  List (Char -> Bool)
  -> Parser Char
satisfyAny = satisfy . (foldRight (lift2 (||)) (pure False))
